...
const AccountCreatedWithGoogle = () => {
  ...
  const [form, setForm] = React.useState({
    firstName: searchParams.get("fistName") || "",
  ...
  const isStep1Filled = form.firstName.trim() !== "" && form.lastName.trim() !== "" && form.phoneNumber.trim() !== "";

  const isStep2Filled = form.isLagosian === "yes" ? form.localGovernment.trim() !== "" : form.isLagosian.trim() !== "";
    ...
    <div className="h-auto flex justify-center py-16">
      <div className="flex flex-col items-center gap-16 relative">
        ...
        <div className="flex flex-col gap-1 w-xl text-center">
          <h1 className="text-2xl font-semibold text-grey-900">Your Account Has Been Created</h1>
          <p className="text-grey-500 font-normal text-lg">You’re in! Now let’s setup your profile in 2 simple steps to personalize your experience.</p>
        </div>
        ...
        <div className="w-[666px] bg-grey-50 flex flex-col gap-12 py-8 px-12 rounded-2xl">
          ...
          <div className="flex flex-col gap-8">
            <p className="text-grey-900 text-base font-normal">
              {step === 1 ? "Help us get to know you better. Fill in your personal details" : "Indigenous Lagosians earn extra points and badges. Confirm your status to unlock special benefits."}
            </p>
            ...
            <form className="flex flex-col gap-6">
              {step === 1 ? (
                ...
                <>
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-normal text-grey-800">Are You an Indigenous Lagosian?</label>
                    <select
                  ...
                  {form.isLagosian === "yes" && (
                    <div className="flex flex-col gap-2">
                      <label className="text-sm font-normal text-grey-800">What Local Government Area are you from?</label>
                      <select
                        value={form.localGovernment}
                        onChange={(e) => setForm({ ...form, localGovernment: e.target.value })}
                        className="w-full border border-grey-100 rounded-full px-3 py-2 text-base text-grey-800 bg-white outline-none">
          ...
          <div className="flex items-center gap-4">
            {step === 2 && (
              <button onClick={handleBack} className="text-sm font-normal text-grey-700 hover:text-primary-500 transition cursor-pointer">
                Go back
              </button>
            )}

            {step === 1 && <p className="text-sm text-grey-500 font-normal cursor-pointer hover:text-primary-500 transition">Skip</p>}

            {step === 1 ? (
              <button
                disabled={!isStep1Filled}
                onClick={handleNext}
                className={`flex items-center gap-2 py-3 px-6 rounded-full text-base font-normal transition ${
                  isStep1Filled ? "bg-primary-500 text-white hover:bg-primary-600" : "bg-grey-100 text-grey-300 cursor-not-allowed"
                }`}>
                Continue
                <ArrowRight2 />
              </button>
              ...
              <button
                ...
                className={`flex items-center gap-2 py-3 px-6 rounded-full text-base font-normal transition ${
                  isStep2Filled ? "bg-primary-500 text-white hover:bg-primary-600" : "bg-grey-100 text-grey-300 cursor-not-allowed"
                }`}>
