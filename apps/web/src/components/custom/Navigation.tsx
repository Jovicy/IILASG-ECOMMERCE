...
import { SearchNormal1, ShoppingCart, NotificationBing, User, ArrowDown2, Trash, ArrowUp2, Gift, TruckFast, Card, Send2 } from "iconsax-reactjs";
    ...
    <div className="sticky top-0 bg-bodies z-40 border-b border-b-grey-100 py-4 px-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-8">
          ...
          {role === "buyer" ? (
            <>
              ...
              <Link to="/buyer/orders" className={navLinkClass}>
                My Orders
              </Link>
              <Link to="/buyer/forums" className={navLinkClass}>
                Forums
              </Link>
            </>
          ) : (
        ...
        <div className="flex items-center gap-3">
          ...
          {role === "buyer" && (
            <button onClick={() => setIsCartOpen(true)} className={`flex items-center gap-1 ${navIconBtnClass}`}>
      ...
      {role === "buyer" && <CartModal isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} />}
    </div>
  );
};
